...
const usersByPet = pet => database.users.filter((user) => user.favPet == pet);
...
const collegeLookup = user => {
  const id = database.users.filter((u) => {
    return u.firstName == user;
  })[0].collegeId;

  const info = database.college.filter((college) => {
...
const local = database.users.filter(({state, collegeId}) => {
  return state == database.college[collegeId].state;
});
console.log(local)
